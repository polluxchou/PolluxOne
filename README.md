...
朗读位置**，而不是固定速度滚动。后置摄像头也能用（拍 b-roll、拍别人），但 HUD 会
明确标出此时提词器已经不在镜头方向上——这是唯一一个「眼神接触」答案为否的状态。
...
- **录制页的实际观感没人看过**：相机链路已在 iPhone 15 Pro 上逐项验证
  （capture session 存活、前后置切换、`.5 · 1× · 3` 光学切换、13/24/76mm 与
  ƒ2.20/1.78/2.80 读数、点击测光与 EV 并存、对焦锁定），但都是通过读取
  `CameraConfiguration` 得到的，**没有人看过屏幕上真正长什么样**——排版、
  可读性、HUD 压在真实画面上的效果仍未评估。
- 设计稿 `Pollux One iOS.dc.html` 未接入：HUD 的视觉决策目前按既有设计语言
  推定，需要 `/design-login` 后逐帧对齐。
